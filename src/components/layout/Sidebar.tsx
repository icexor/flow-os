...
export function AppSidebar({ currentPage, onNavigate, collapsed, onToggle, onLogout }: SidebarProps) {
  return (
    <aside
      className={cn(
            ...
            return (
              <li key={item.id}>
                <button
                  ...
                  className={cn(
                    "w-full flex items-center rounded-md transition-all duration-150 outline-none group",
        ...
        {!collapsed && (
          <>
            ...
            <div className="px-2">
              ...
              <button
                onClick={() => onNavigate("ai-dataflow")}
                className={cn(
                  ...
                  currentPage === "ai-dataflow" && "bg-primary/15 text-primary border-primary/20"
                )}
              >
                <Activity className="w-4 h-4" />
                <span className="text-sm font-medium">AI Data Flow</span>
              </button>
              <button
                onClick={() => onNavigate("business-intelligence")}
                className={cn(
                  "w-full flex items-center gap-3 px-3 py-2 rounded-md transition-all duration-150 text-sidebar-foreground/60 hover:bg-sidebar-accent hover:text-sidebar-foreground border border-transparent",
                  currentPage === "business-intelligence" && "bg-primary/15 text-primary border-primary/20"
                )}
              >
                <BrainCircuit className="w-4 h-4" />
                <span className="text-sm font-medium">Business Intel</span>
              </button>
            </div>
          </>
        )}
        ...
        {!collapsed ? (
          <div className="flex items-center gap-3 px-1 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-foreground truncate">Arjay Delos Santos</p>
